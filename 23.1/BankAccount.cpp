...
MyBankAccount::MyBankAccount(string name, double chkBalance, double savBalance, double fee) : BankAccount(name, chkBalance, savBalance), transactionFee(fee) {}

void MyBankAccount::WithdrawSavings(double amt){
   double totalAmt = amt + transactionFee;

   if(totalAmt <= GetSavBalance()){
      SetSavBalance(GetSavBalance() - totalAmt);
   }
}

void MyBankAccount::TransferToChecking(double amt){
   double totalAmt = amt + transactionFee;
   if(totalAmt <= GetSavBalance()){
      SetSavBalance(GetSavBalance() - totalAmt);
      SetChkBalance(GetChkBalance() + amt);
   }
}